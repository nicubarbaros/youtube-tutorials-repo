...
module.exports = async (program, directory, suppliedStage, port, {
  ...
  process.env.GATSBY_BUILD_STAGE = suppliedStage === `build-ssr` ? `build-html` : suppliedStage; // We combine develop & develop-html stages for purposes of generating the
  // webpack config.
  ...
  const {
    ...
    loaders,
    plugins
  } = (0, _webpackUtils.createWebpackUtils)(suppliedStage === `build-ssr` ? `build-html` : stage, program);
  const {
    assetPrefix,
    pathPrefix
  ...
  function processEnv(stage, defaultNodeEnv) {
    ...
    envObject.BUILD_STAGE = JSON.stringify(stage === `build-ssr` ? `build-html` : stage);
  ...
  function getResolve(stage) {
    ...
    const target = stage === `build-html` || stage === `develop-html` || stage === `build-ssr` ? `node` : `web`;
  ...
  if (stage === `build-html` || stage === `develop-html` || stage === `build-ssr`) {
  ...
  if (stage === `build-html` || stage === `develop-html` || stage === `build-ssr`) {
    ...
    const shouldMarkPackagesAsExternal = stage === `develop-html` || !("" === `4` && (0, _enginesHelpers.shouldGenerateEngines)()); // tracking = build-html (when not generating engines)

    const shouldTrackBuiltins = stage === `build-html` && !("" === `4` && (0, _enginesHelpers.shouldGenerateEngines)()); // removes node internals from bundle
  ...
  if (stage === `build-javascript` || stage === `build-html` || stage === `build-ssr` || process.env.GATSBY_EXPERIMENTAL_DEV_WEBPACK_CACHE && (stage === `develop` || stage === `develop-html`)) {
