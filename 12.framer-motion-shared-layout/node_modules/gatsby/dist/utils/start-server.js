...
module.exports = {
  ...
  let waitForServerCompilation = () => Promise.resolve(undefined);

  if ("" === `4`) {
    const serverDevConfig = await (0, _webpack2.default)(program, directory, _types.Stage.SSR, program.port, {
      parentSpan: webpackActivity.span
    });
    const serverCompiler = (0, _webpack.default)(serverDevConfig);
    const waitForServerCompilationDeferred = (0, _pDefer.default)();

    waitForServerCompilation = () => waitForServerCompilationDeferred.promise;
    ...
    serverCompiler.watch({
      ...
      if (compileCounter++ > 0) {
        waitForServerCompilation = () => Promise.resolve(stats);
      } else {
    ...
    if (page) {
      try {
        ...
        if (page.mode === `SSR`) {
          ...
          if (!stats || !stats.compilation.entrypoints.has(page.componentChunkName)) {
            _reporter.default.error(`Error loading a result for the page query in "${requestedPagePath}" / "${potentialPagePath}". getServerData threw an error.`);
          } // eslint-disable-next-line @typescript-eslint/no-non-null-assertion


          const componentHash = stats.compilation.entrypoints.get(page.componentChunkName).getEntrypointChunk().hash;
          const modulePath = path.resolve(`${program.directory}/${_constants.ROUTES_DIRECTORY}/${page.componentChunkName}.js`); // if webpack compilation is diff we delete old cache

          if (previousHashes.has(page.componentChunkName) && previousHashes.get(page.componentChunkName) === componentHash) {
          ...
          serverDataPromise = (0, _getServerData.getServerData)(req, page, potentialPagePath, mod);
        }
